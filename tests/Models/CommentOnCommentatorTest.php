...
it('can add comments', function ($transformer, $text) {
    ...
    $this->user->comment('my comment');

    expect($this->user->comments)->toHaveCount(1);

    expect($this->user->comments->first())
...
it('can add multiple comments', function () {
    $this->user->comment('comment 1');
    $this->user->comment('comment 2');
    $this->user->comment('comment 3');

    expect($this->user->comments)->toHaveCount(3);
});

it('can nest comments', function () {
    $comment = $this->user->comment('comment 1');
    $comment->comment('comment 2');

    expect($this->user->comments)->toHaveCount(1);
    expect($this->user->comments->first()->comments)->toHaveCount(1);
});
...
it('can create a comment for a specific user', function () {
    ...
    $this->user->comment('my comment', $anotherUser);

    expect($this->user->comments->first()->commentator)->isModel($anotherUser);
});

it('can create a nested comment', function () {
    $this->user->comment('top level comment');
...
it('can get the commentator properties', function () {
    $comment = $this->user->comment('comment');
...
it('will determine that an anonymous comment was not made by a deleted user', function () {
    ...
    $comment = $this->user->comment('my comment', null);
    expect($comment->commentator)->toBeNull();
...
it('will determine that a comment by a deleted user was not made by an anonymous user', function () {
    $comment = $this->user->comment('comment by current user');
